...
import type { ToolResponse, TokenTransferTransaction } from '@/lib/ai/tools/types';

interface TokenTransferToolResult {
...
interface TokenTransferCardProps {
  data: ToolResponse<TokenTransferToolResult>;
}
...
export function TokenTransferCard({ data }: TokenTransferCardProps) {
  ...
  if (transaction && !txHash) {
    return (
      <BaseCard title="Token Transfer" icon={<Coins className="w-4 h-4" />} variant="default">
        <TransactionWrapper transaction={transaction} buttonText="Sign & Transfer">
          <div className="space-y-2 mb-3">
            <DataRow
              label="Amount"
              value={`${transaction.amount} ${transaction.symbol || 'tokens'}`}
              highlight
            />
            <AddressDisplay address={transaction.to} label="To" />
            <AddressDisplay address={transaction.tokenAddress} label="Token" />
          </div>
        </TransactionWrapper>
      </BaseCard>
    );
  ...
  return <ErrorCard error="Invalid transfer data" toolName="Token Transfer" />;
}
